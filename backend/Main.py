    ...
    if coords[0] != None and coords[1] != None:
        ...
        while not stopped:
            ...
            if not squareY <= coords[1] < squareY + dy:
                squareY += dy
                stopped = False
        temp_copy = cv2.rectangle(temp_copy, (int(squareX), int(squareY)), (int(squareX + dx), int(squareY + dy)),
                                  (100, 100, 100), -1)
        ...
        if square != previous_square and str(square) in squares.keys():
            ...
            if filename != None:
                playsound("./MIDI_files/" + filename + ".mp3", False)
...
class MediaPipe_2Hands(Resource):
    ...
    dic = {"finger1": finger1, "finger2": finger2, "finger3": finger3, "finger4": finger4, "finger5": finger5,
           "finger6": finger6, "finger7": finger7, "finger8": finger8}

    def get(self):
        img = image()
        frame = MediaPipe2Hands(img, self.dic["finger1"], self.dic["finger2"], self.dic["finger3"], self.dic["finger4"],
                                self.dic["finger5"], self.dic["finger6"], self.dic["finger7"], self.dic["finger8"])
        result = prepareResultMediaPipeHands(frame, [0, 0])
        return result
